...
declare -A DEPENDENCIES=(
	...
	[$REPOSITORY_NAME]=$REPOSITORY_URL" "$REPOSITORY_BRANCH
)

CORE=$REPOSITORY_NAME
REV=$REPOSITORY_REV

#─────────────────────────────────────────────────────────────────────────────
# Prepare build and output directory structure

BUILD_DIR=$SELF_DIR/_build
mkdir -p "$BUILD_DIR"
OUT_DIR="$(readlink -f "$BUILD_DIR/output")"
mkdir -p "$OUT_DIR"
OUT_CORE_DIR="$(readlink -f "$OUT_DIR/$CORE")"
mkdir -p "$OUT_CORE_DIR"
ARTIFACTS_DIR="$(readlink -f "$OUT_CORE_DIR/artifacts")"
